            ...
            <div class="container flex justify-between mx-auto">
                <div class="w-full lg:w-8/12">
                    <div class="flex items-center justify-between">
                        <h1 class="text-xl font-bold text-gray-700 md:text-2xl">Notes</h1>
                        <div>
                            <select
                                class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                <option>Récents</option>
                                <option>Semaines précédentes</option>
                            </select>
                        </div>
                    </div>
                    <div class="mt-6">
                        <div class="max-w-4xl px-10 py-6 mx-auto bg-white rounded-lg shadow-md">
                            <div class="flex items-center justify-between">
                                @forelse ($notes as $note)
                                    <span class="font-light text-gray-600">{{ $note->created_at->format('d') }}
                                        {{ $note->created_at->format('M') . ' ' . $note->created_at->format('Y') }}</span><a
                                        href="#"
                                        class="px-2 py-1 font-bold text-gray-100 bg-gray-600 rounded hover:bg-gray-500">{!! $note->tag !!}</a>
                            </div>
                            <div class="mt-2">
                                ...
                                <p class="mt-2 text-gray-600">{!! $note->tag !!}</p>
                            @empty
                            ...
                            <div class="flex items-center justify-between mt-4"><a href="#"
                                ...
                                <div><a href="#" class="flex items-center">
                                        <h1 class="font-bold text-gray-700 ">{!! $note->user->name !!}</h1>
                                    </a></div>
                                @endforelse
                            </div>
                        </div>
                            ...
                            <ul class="-mx-4">
                                @forelse ($notes as $note)
                                    <li class="flex items-center">
                                        <p class="font-bold">{!! $note->user->name !!}</p>
                                    @empty
                                        <p class="text-center font-bold mx-auto my-3">Il n'y aucun auteur</p>
                                    </li>
                                    <span class="text-sm font-light text-gray-700">possède
                                        {{ count(Auth::user()->notes) }} note</span>
                                @endforelse
                            </ul>
                        </div>
                    </div>
                            ...
                            @forelse ($notes as $note)
                                <ul>
                                    <li><a href="#"
                                            ...
                                            {!! $note->tag !!}</a></li>
                                @empty
                                    <p class="text-center font-bold mx-auto my-3">Pas de tag créer</p>
                                </ul>
                            @endforelse
                        </div>
                    </div>
                    <div class="px-8 mt-10">
                        <h1 class="mb-4 text-xl font-bold text-gray-700">Notes Récentes</h1>
